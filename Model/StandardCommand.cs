...
namespace Model
{
    ...
    {
        public class StanderdExpressionProcessor : ExpressionProcessor
        {
            private static StanderdExpressionProcessor _instance;
            private StanderdExpressionProcessor()
            {
                CurrentBase = NumberBase.Decimal;
            }
            public static StanderdExpressionProcessor instance
            {
                get
                {
                    if (_instance == null)
                    {
                        _instance = new StanderdExpressionProcessor();
                    }
                    return _instance;
                }
            }
        ...
        public class EqualOperation : IOperation
        {
            ...
            public string Execute(string op)
            {
                ...
                //{
                //StanderdExpressionProcessor processor = new StanderdExpressionProcessor();
                try
                    {
                        op = StanderdExpressionProcessor.instance.EvaluateRPN(
                             StanderdExpressionProcessor.instance.ConvertToPostfix(
                             StanderdExpressionProcessor.instance.TokenizeExpression(op))).ToString("0.#########");
                    }
                    catch (Exception ex)
                    {
                        op = "Error";
